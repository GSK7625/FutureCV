  ...
  return (
    <section aria-labelledby="dashboard-title">
      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <Card><CardContent className="p-6"><p className="text-label font-semibold text-gold">APPLICATION VÀ PIPELINE</p><h2 className="mt-4 text-body-lg font-semibold text-navy">Chưa có API backend</h2><p className="mt-2 text-label text-ink-variant">Số liệu ứng viên, MatchResult và lịch phỏng vấn không được hiển thị cho tới khi backend cung cấp hợp đồng API.</p><Link to="/hr/pipeline" className="mt-4 inline-flex items-center gap-2 text-label font-semibold text-navy hover:text-gold">Xem danh sách API cần bổ sung<IconArrowRight size={17} aria-hidden="true" /></Link></CardContent></Card>
      </div>
    </section>
  );
}
