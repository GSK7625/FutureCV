...
export function EditJobModal({ job, onClose }: EditJobModalProps) {
  ...
  const [formData, setFormData] = useState<UpdateJobRequest>({
    ...
    experienceLevel: job.levelName || '',
    employmentType: job.employmentTypeName || '',
    closingDate: job.deadline || '',
  });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const handleChange = (field: keyof UpdateJobRequest, value: any) => {
  ...
  const validate = (): boolean => {
    ...
    if (formData.salaryMax !== undefined && formData.salaryMin !== undefined && formData.salaryMax < formData.salaryMin) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-slate-900 border-b border-slate-800 px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-slate-50">Chỉnh sửa tin tuyển dụng</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            ...
            >
              {updateJob.isPending ? 'Đang xử lý...' : 'Cập nhật'}
            </button>
          </div>
        </form>
      </div>
